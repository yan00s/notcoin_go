...
func (notcoin *Notcoin) Set_default_values() {
	notcoin.LimitCoins = get_randomint(148, 170, 1)
	notcoin.LastAvailableCoins = get_randomint(1, 147, 1)
...
func (notcoin *Notcoin) work(wg *sync.WaitGroup) {
	...
	InfoLogger.Printf("[%v] Starting thread userid = %v, Proxy = %v", userid, userid, notcoin.Proxy)
	...
	for count_iteration := 0; count_iteration != -1; count_iteration++ {
		...
		if notcoin.LastAvailableCoins >= 100 || is_slept || notcoin.Turbo {
			...
			notcoin.LastAvailableCoins += get_randomint(1, 30, 1)
			sleep_time = get_randomint(51, 98, 1)
		...
		sleep_time = get_randomint(2, 13, 1)
...
func (notcoin *Notcoin) setreqwebappdata(raw_data string) {
	...
	decoded_url, _ := url.QueryUnescape(split2)
	notcoin.TGWebAppData = decoded_url
}
...
func (notcoin *Notcoin) SetSession() error {
	...
	raw_tgwebappdata, err := notcoin.get_appdata()
	if err != nil {
...
// TODO:
// сделац добавление говна в accounts.json

// придать логам юзерфрендли вид ++
// чекнуть вокинг проксей ++
// реализовать покупку апгрейдов ++
// реализовать покупку турбо хреней ++
// сделать нормальный запуск потоков ++
