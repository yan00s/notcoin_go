...
type Notcoin struct {
	...
	Count_400          int
	LimitCoins         int   // max coins
	LastAvailableCoins int   // остаток для фарма, надо обновлять после каждого клика
	BalanceCoins       int   // надо обновлять после каждого клика
	Turbo              bool  // турбо в респонсе, надо обновлять после каждого клика и если тру то делать кеф больше
	Timestart_turbo    int64 // начало турбо
	Turbo_boost_count  int
	// 2800 = 5сек, кеф 5, (3 запроса за эти же 5 сек)// не может превышать максимум за один запрос
	Hash int
}
